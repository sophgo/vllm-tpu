...
class AsyncTPUModelRunnerOutput(AsyncModelRunnerOutput):
    ...
    def __init__(
        ...
        async_output_copy_stream: torch_tpu.tpu.Stream,
    ):
...
class SophTPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self.use_tpugraph = self._use_tpugraph()
        self.tpugraph_batch_sizes = list(
            reversed(self.compilation_config.cudagraph_capture_sizes)) if self.compilation_config.cudagraph_capture_sizes is not None else None

        self.uniform_decode_query_len = 1 if not self.speculative_config else \
            1 + self.speculative_config.num_speculative_tokens
        # tpugraph dispatcher for runtime tpugraph dispatching.
        self.tpugraph_dispatcher = CudagraphDispatcher(self.vllm_config)
        # Cached outputs.
    ...
    def execute_model(
        ...
        tpugraph_runtime_mode, batch_descriptor = \
            self.tpugraph_dispatcher.dispatch(batch_descriptor)
    ...
    def _build_dummy_attn_metadata(
        ...
        if force_attention or tpugraph_runtime_mode == CUDAGraphMode.FULL:
    ...
    def _dummy_run(
        ...
        assert tpugraph_runtime_mode is None or tpugraph_runtime_mode in {
            ...
            _tg_mode, batch_descriptor = \
                self.tpugraph_dispatcher.dispatch(
                    BatchDescriptor(num_tokens=num_tokens,
                                    uniform_decode=uniform_decode))
            if tpugraph_runtime_mode is not None:
    ...
    def load_model(self) -> None:
        ...
        # Cache mode is disabled during load_model prpcess, and this only causes a negligible
        # preformance impact during model initialization.
    ...
    def initialize_kv_cache_tensors_deepseek_sfa(
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                num_blocks, block_size, num_kv_heads, head_size = kv_cache_shape[1:]
                rope_dim = self.model_config.hf_text_config.qk_rope_head_dim
    ...
    def initialize_kv_cache_tensors_deepseek_mla(
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                #rope_dim = self.model_config.hf_text_config.qk_rope_head_dim
                #nope_dim = head_size - rope_dim
                tpu_kv_cache_shape = (num_blocks, block_size, kv_lora_rank)
                tpu_pe_cache_shape = (num_blocks, block_size, qk_rope_head_dim)
                if self.vllm_config.kv_transfer_config is None:
                    # For no disaggregate pd scenario, allocate kv cache in normal way
                    tpu_kv_cache = torch.zeros(tpu_kv_cache_shape,
                                               dtype=dtype,
                                               device=self.device)
                    tpu_pe_cache = torch.zeros(tpu_pe_cache_shape,
                                               dtype=dtype,
                                               device=self.device)
                else:
                    ...
                    kv_allocate_shape_alignment = kv_allocate_shape + alignment
                    pe_allocate_shape = num_blocks * block_size * qk_rope_head_dim
                    pe_allocate_shape_alignment = pe_allocate_shape + alignment

                    tpu_kv_cache = torch.zeros(kv_allocate_shape_alignment,
                                               dtype=dtype,
                                               device=self.device)
                    tpu_pe_cache = torch.zeros(pe_allocate_shape_alignment,
                                               dtype=dtype,
                                               device=self.device)
                    tpu_kv_cache = self._align_memory(
                        tpu_kv_cache,
                        alignment)[:kv_allocate_shape].view(tpu_kv_cache_shape)
                    tpu_pe_cache = self._align_memory(
                        tpu_pe_cache,
                        alignment)[:pe_allocate_shape].view(tpu_pe_cache_shape)
                ...
                if "linear_attn" in layer_name:
                    ...
                    for layer_name_inner in kv_cache_tensor.shared_by:
                        if ("attn" in layer_name_inner and "linear_attn" not in layer_name_inner) or \
                            ...
                            continue
                        if self.vllm_config.kv_transfer_config is None:
                            tensor = torch.empty(kv_cache_tensor.size,
                                                 dtype=torch.int8,
                                                 device=self.device)
                        else:
                            cache_size_aligned = kv_cache_tensor.size + alignment
                            tensor = torch.empty(cache_size_aligned,
                                                 dtype=torch.int8,
                                                 device=self.device)
                    ...
                    if self.vllm_config.kv_transfer_config is None:
                        k_tensor = torch.empty(kv_cache_tensor.size // 2,
                                               dtype=torch.uint8,
                                               device=self.device)
                        v_tensor = torch.empty(kv_cache_tensor.size // 2,
                                               dtype=torch.uint8,
                                               device=self.device)
                    else:
                        cache_size = kv_cache_tensor.size // 2
                        cache_size_aligned = kv_cache_tensor.size // 2 + alignment
                        k_tensor = torch.empty(cache_size_aligned,
                                               dtype=torch.int8,
                                               device=self.device)
                        v_tensor = torch.empty(cache_size_aligned,
                                               dtype=torch.int8,
                                               device=self.device)
    ...
    def initialize_tpugraph_capture(self) -> None:
        ...
        self.tpugraph_dispatcher.initialize_cudagraph_keys(
            self.compilation_config.cudagraph_mode,
            self.uniform_decode_query_len)

    def _capture_tpugraphs(self, compilation_cases: list[int],
                           tpugraph_runtime_mode: CUDAGraphMode,
                           uniform_decode: bool):
        assert tpugraph_runtime_mode != CUDAGraphMode.NONE and \
            tpugraph_runtime_mode in [CUDAGraphMode.FULL,
                                      CUDAGraphMode.PIECEWISE]
    ...
    def capture_model(self):
        ...
        with graph_capture(device=self.device):
            ...
            if tpugraph_mode.mixed_mode() != CUDAGraphMode.NONE:
                tpugraph_runtime_mode = tpugraph_mode.mixed_mode()
            ...
            if tpugraph_mode.decode_mode() == CUDAGraphMode.FULL and \
                ...
                decode_cudagraph_batch_sizes = [
                    x for x in self.tpugraph_batch_sizes if x <= max_num_tokens
                ...
                self._capture_tpugraphs(
                    ...
                    tpugraph_runtime_mode=CUDAGraphMode.FULL,
                    uniform_decode=True)

        # Disable tpugraph capturing globally, so any unexpected tpugraph
