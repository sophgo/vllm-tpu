...
from vllm.sophtpu_utils import sanity_check_mm_encoder_outputs
...
class SophTPUModelRunner:
    ...
    def _prepare_prompt(
        ...
        #这个input_positions应该就是gpu里面的positions
        # 处理位置信息
        if self.uses_mrope:
            # 创建GPU上的M-RoPE位置张量
        ...
        for i, req_id in enumerate(req_ids):
            ...
            if self.uses_mrope:
                ...
                if num_computed < num_prompt_tokens:
                    # 还在处理prompt部分，使用预计算的M-RoPE位置
                    mrope_positions_cpu[:, i] = req.mrope_positions[:, num_computed]
                else:
                    # 处理completion部分，动态计算M-RoPE位置
