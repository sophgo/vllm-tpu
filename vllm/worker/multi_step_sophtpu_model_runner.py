...
class ModelInputForSophTPU(ModelRunnerInputBase):
    ...
    def from_broadcasted_tensor_dict(
        cls: Type["ModelInputForSophTPU"],
        tensor_dict: Dict[str, Any],
        attn_backend: Optional["AttentionBackend"] = None,
    ) -> "ModelInputForSophTPU":
        if attn_backend is not None:
            tensor_dict = _init_attn_metadata_from_tensor_dict(
                attn_backend, tensor_dict)
        return cls(**tensor_dict)

class SophTPUModelRunner(ModelRunnerBase[ModelInputForSophTPU]):
    ...
    def _prepare_prompt(
        ...
        # Initialize lists for input processing
        input_tokens = []
        input_positions = []
        prompt_lens = []
        context_lens = []
        ...
        for batch_idx, seq_group_metadata in enumerate(seq_group_metadata_list):
            assert seq_group_metadata.is_prompt, "Metadata must be a prompt."

            # Extract sequence ID and data
            seq_ids = list(seq_group_metadata.seq_data.keys())
            assert len(seq_ids) == 1, "Each group must contain exactly one sequence ID."
            seq_id = seq_ids[0]
            seq_data = seq_group_metadata.seq_data[seq_id]

            # Retrieve and process prompt tokens
            prompt_tokens = seq_data.get_token_ids()
            seq_len = len(prompt_tokens)
            ...
            # Handle block tables and slot mapping
            assert seq_group_metadata.block_tables is not None, "Block tables must be provided."
            block_table = seq_group_metadata.block_tables[seq_id]
            for i in range(num_computed_tokens, seq_len):
                block_number = block_table[i // self.block_size]
                block_offset = i % self.block_size
            ...
            # Tokenized input and related lengths
            tokenized_input = prompt_tokens
            input_length = len(tokenized_input)

            # Paged attention setup
            max_new_tokens = 10
            speculative_length = 0
        ...
        # Final input tensor conversions
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            if sampling_params.top_p != 1 and not _ENABLE_TOP_P:
                raise NotImplementedError(
                    "Top-p sampling is currently disabled for the Sophon TPU backend "
                    "due to performance issues.")
            p.append(sampling_params.top_p)
            if sampling_params.top_k != -1:
                raise NotImplementedError(
                    "Top-k sampling is currently disabled for the Sophon TPU backend "
                    "due to performance issues.")
            if sampling_params.n > _MAX_NUM_SAMPLES:
                raise NotImplementedError(
                    f"Best of > {_MAX_NUM_SAMPLES} is not supported by the Sophon TPU "
                    "backend.")
            n.append(sampling_params.n)
            if sampling_params.logprobs is not None:
                raise NotImplementedError(
                    "logprobs is not currently supported by the Sophon TPU backend.")
            if sampling_params.prompt_logprobs is not None:
                raise NotImplementedError(
                    "prompt_logprobs is not currently supported by the Sophon TPU "
                    "backend.")
    ...
    def prepare_model_input(
        ...
        return ModelInputForSophTPU(input_tokens, input_positions, attn_metadata,
                                    input_lens, t, p, num_samples, n, seq_groups)

    def make_model_input_from_broadcasted_tensor_dict(
            self, tensor_dict: Dict[str, Any]) -> ModelInputForSophTPU:
        model_input = ModelInputForSophTPU.from_broadcasted_tensor_dict(
    ...
    def execute_model(
        self,
        model_input: ModelInputForSophTPU,
