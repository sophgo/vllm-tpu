...
TModelInputForSophTPU = TypeVar('TModelInputForSophTPU', bound="ModelInputForSophTPU")

@dataclass(frozen=True)
class ModelInputForSophTPU(ModelRunnerInputBase):
    ...
    def from_broadcasted_tensor_dict(
        cls: Type[TModelInputForSophTPU],
        tensor_dict: Dict[str, Any],
        attn_backend: Optional["AttentionBackend"] = None,
    ) -> TModelInputForSophTPU:
...
@dataclass(frozen=True)
class ModelInputForSophTPUWithSamplingMetadata(ModelInputForSophTPU):
    """
    ...
    def from_broadcasted_tensor_dict(
        ...
        tensor_dict: Dict[str, Any],
        attn_backend: Optional["AttentionBackend"] = None,
    ) -> "ModelInputForSophTPUWithSamplingMetadata":
...
class ModelInputForSophTPUBuilder(ModelRunnerInputBuilderBase[ModelInputForSophTPU]):

    def __init__(self,
                 runner: "SophTPUModelRunner",
        ...
        self.max_num_blocks_per_seq = (self.runner.model_config.max_model_len //
                                       self.block_size)
        import numpy as np
        self.block_tables = np.zeros(
            (self.runner.scheduler_config.max_num_seqs, self.max_num_blocks_per_seq),
            dtype=np.int32)
    ...
    def build(self) -> ModelInputForSophTPU:
    ...
    def _prepare_prompt(
        ...
        for batch_idx, seq_group_metadata in enumerate(seq_group_metadata_list):
            ...
            num_computed_tokens = seq_data.get_num_computed_tokens()

            if num_computed_tokens > 0:
                prompt_tokens = prompt_tokens[num_computed_tokens:]
                context_lens.append(seq_len)  # Prompt token num
            else:
            ...
            positions_range = range(num_computed_tokens, seq_len)
            ...
            request_blocks = list(range(num_blocks, num_blocks + needed_blocks))
...
class SophTPUModelRunner(ModelRunnerBase[ModelInputForSophTPUWithSamplingMetadata]):
    _model_input_cls: Type[ModelInputForSophTPUWithSamplingMetadata] = (
        ModelInputForSophTPUWithSamplingMetadata)
    _builder_cls: Type[ModelInputForSophTPUBuilder] = ModelInputForSophTPUBuilder
    ...
    def make_model_input_from_broadcasted_tensor_dict(
            ...
            tensor_dict: Dict[str,
                              Any]) -> ModelInputForSophTPUWithSamplingMetadata:
        return (
            ModelInputForSophTPUWithSamplingMetadata.from_broadcasted_tensor_dict(
                tensor_dict,
    ...
    def execute_model(
        self,
        model_input: ModelInputForSophTPUWithSamplingMetadata,
        ...
        if num_steps > 1:
            raise ValueError(
                "SophTPUModelRunner does not support multi-step execution.")

        #is_prompt = model_input.attn_metadata.num_prefills > 0
        #if is_prompt:
        #    token_ids = model_input.token_ids.unsqueeze(dim=0).to(self.device)
        #else:
        #    token_ids = model_input.token_ids.to(self.device)
        #position_ids = model_input.position_ids.to(self.device)
        #attn_metadata = model_input.attn_metadata
        #attn_metadata.num_prefills = model_input.input_lens.shape[0]
        ...
        ##attn_metadata.effective_query_lens = attn_metadata.effective_query_lens.to(self.device)
...
class ModelWrapper(nn.Module):
    ...
    def forward(
        ...
        token_ids = token_ids.flatten(0, 1)
        ...
        sampled_token_ids = torch.multinomial(probs,
                                              num_samples,
                                              replacement=True)
        sampled_token_ids = sampled_token_ids.to(device = device, dtype = torch.int32)
        if num_samples == 1:
